...
export class CleanerSignUpComponent implements OnInit {
  placesArray: BehaviorSubject<GooglePlacesAutocomplete[]>;
  placesAutocomplete: GooglePlacesAutocomplete;
  isListViewVisible: boolean = false;
  displayListView = false;
  autocompleteResults: BehaviorSubject<string[]> = new BehaviorSubject<string[]>([]);
  ...
  // }
constructor(@Inject(RouterExtensions) private router: RouterExtensions) {}
  ngOnInit() {
  ...
  onAddressInputChange(args) {
    ...
    this.placesAutocomplete.search(textField.text).then(
      (places: any) => {
        ...
        this.placesArray = new BehaviorSubject<GooglePlacesAutocomplete[]>(
          predictions
        );
      },
      (error) => {
  ...
  onItemTap(args: ItemEventData) {
    ...
    const selectedAddress = this.placesArray.value[args.index];
