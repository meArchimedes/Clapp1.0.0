...
import { Validators, EmailValidator, FormControl, FormGroup } from "@angular/forms";
...
export class CleanerSignUpComponent implements OnInit {
  ...
  emailControlIsValid: boolean = false;
  ...
  isEmailValid(){
    if (/^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$/.test(this.email)){}      this.emailControlIsValid = true;
    }
